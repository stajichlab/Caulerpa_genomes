#!/bin/bash
#SBATCH -N 1 -n 24 --mem 96gb --out logs/spades.%a_%A.log

module load AAFTF/git-live
module switch SPAdes/3.14.1
MEM=96
OUTDIR=asm_spades
SAMPLES=samples.csv
...
do
	...
	if [ ! -f $LEFT ]; then
		...
		AAFTF trim --mem $MEM --method bbduk --left $INLEFT  --right $INRIGHT -o $WORKDIR/${PREFIX} -c $CPU
 	    ...
 	    AAFTF filter --mem $MEM -c $CPU --left $LEFTTRIM --right $RIGHTTRIM --aligner bbduk -o $WORKDIR/${PREFIX}
	fi
...
#	PILON=$OUTDIR/${PREFIX}.pilon.fasta
	...
	if [ ! -f $ASMFILE ]; then
	    AAFTF assemble --mem $MEM --left $LEFT --right $RIGHT -o $ASMFILE -c $CPU --method spades --assembler_args="--meta" --debug
	fi
	if [ ! -f $VECCLEAN ]; then
